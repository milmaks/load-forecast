...
class DataPreprocessor:
                    ...
                    if ((load_object['Time Stamp'])[-5:]).strip() == '00:00' and load_object['Name'].strip() == self.town:
                        ...
                        if current_day == None:
                            ...
                            if current_day == datetime.strptime(load_object['Time Stamp'], '%m/%d/%Y %H:%M:%S').strftime('%Y-%m-%d'):
                                ...
                                self.database.add_average_load(int(current_day[0:4]), int(current_day[5:7]), int(current_day[8:10]), avg)
                                one_day_load.clear()
                                one_day_load.append(load_object['Load'])
                                current_day = datetime.strptime(load_object['Time Stamp'], '%m/%d/%Y %H:%M:%S').strftime('%Y-%m-%d')
            ...
            for file in dirs[2]:
                ...
                for i in range(len(weather_data.index)):
                    ...
                    if nonCompleteExists:
                        ...
                        self.write_to_db(lastNonCompleteModel)
                        nonCompleteExists = False
    ...
    def write_to_db(self, learningModel: LearningModel):
        ...
        self.database.add_element(learningModel)
        # if weather_object['solarradiation'] == NaN:
        #     weather_object['solarradiation'] = 0
    ...
    def calculate_daylight(self, datetimeLight):
        ...
        #print(elevation(city.observer, est.localize(datetime.strptime(datetimeLight, '%Y-%m-%dT%H:%M:%S')), False))
